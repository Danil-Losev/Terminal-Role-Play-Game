...
void PERSON::setPerson(int index)
{
    ...
    for (int i = 0; i < PERSON_STAT_COUNT; i++) {
        printByChar("  " + std::to_string(i + 1) + ')' + statName[i] + std::to_string(this->*stats[i]) + '\n',
            40);
    }
    ...
    while (getChar != '\r' && getChar != '\n') {
        ...
        if ((getChar == 'w' || getChar == 72) && curStat > 0) {
            curStat--;
        } else if ((getChar == 's' || getChar == 80) && curStat < PERSON_STAT_COUNT - 1) { // Исправили условие
            curStat++;
...
void gameSettings()
{
    ...
    while (getChar != '\r' && getChar != '\n') {
        ...
        if ((getChar == 'w' || getChar == 72) && chose > 0) {
            ...
            chose++;
        } else if ((getChar == 'd' || getChar == 77) && *tempSetArray[chose] < 100) {
            (*tempSetArray[chose])++;
...
void hideCursor()
{
    HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE); // Получить дескриптор консоли
